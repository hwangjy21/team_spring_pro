...
document.addEventListener("click", (e) => {
    if(e.target.id == 'delBtn'){
        ...
        console.log("하트버튼 누르면 여기와야해")
        let iconContainer = document.getElementById("iconContainer"); //아이콘 넣을 div
        if (reviewWriter != authId) {
            likeQtyToServer(rnoVal, authId)
                .then(result => {
                    ...
                    return likeQtyAreaInput(rnoVal);
                })
                .then(result => {
...
//리뷰 좋아요
async function likeQtyToServer(rno,id){
    try {
        const url = "/review/reviewLike/"+rno+"/"+id;
        const config = {
            method: "post"
        };
...
//리뷰 좋아요 수 가져오기 
async function likeQtyAreaInput(rno){
    try {
        console.log("좋아요수 가져오는 곳")
        const url = "/review/likeQtyAreaInput/"+rno;   
        const resp = await fetch(url);
        const result = await resp.text();  
...
//댓글 요청 함수
async function spreadCommentListFromServer(rno, page) {
    try {
        const url = "/comment/reviewCommentlist/" + rno + '/' + page;
        const resp = await fetch(url);
 ...
 //댓글 리스트 출력 함수
 function printCommentList(rnoVal, page = 1) { //page=1인거는 처음 뿌릴 때는 무조검 첫페이지 뿌리라고 한거(옵셔널 값은 가장 마지막에 입력)
    spreadCommentListFromServer(rnoVal, page).then(result => {
        ...
        if (result.cdtoList.length > 0) {
            ...
            for (let cdto of result.cdtoList) {
                ...
                if(cdto.cvo.id==reviewWriter){
                    str += `<span style="margin-left:10px;"class="badge rounded-pill text-bg-success">작성자</span>`;
                }
                str += `<span style="margin-right:10px;"class="miniFont dateText">${cdto.cvo.id}</span><span style="color:rgb(221, 221, 221);">|</span><span class="miniFont dateText">${cdto.cvo.regDate}</span></div>`;   
...
document.addEventListener('click', (e) => {
    ...
    //수정 등록(modPostBtn)
        ...
        printCommentList(rnoVal, parseInt(e.target.dataset.page));
